...
const progressPersonal = document.querySelector(
  ".first-square-personal-inside"
);
const progressChess = document.querySelector(".second-square-personal-inside");
const participatedQuestion = document.querySelectorAll(
  ...
  button.addEventListener("click", (e) => {
    ...
    switch (index) {
      ...
      case 1: // Personal Information page
        ...
        // Email
        if (
          data.email.split("@")[1] === "redberry.ge" &&
          data.email.split("@")[0].length > 2
        ) {
  ...
  .then((data) => {
    data.forEach((record, index) => {
      ...
      boxOption.innerHTML =
        record.name +
        `<img
            src="https://chess-tournament-api.devtest.ge/` +
        record.image +
        `">`;
  ...
  switch (input.name) {
    ...
    case "email":
      setLocal(
        input,
        1,
        input.value.split("@")[1] === "redberry.ge" &&
          input.value.split("@")[0].length > 2
      );
      break;
...
const validArray = [
  ...
  email.value.split("@")[1] === "redberry.ge" &&
    email.value.split("@")[0].length > 2,
